...
const Error = () => {
  const [score, setScore] = useState(4568)
      ...
      <div className={'oneArticle'}>
        <div className="container">
          ...
          <div className="oneArticle__img-container">
            <img className={'oneArticle__img'} src="/images/error.jpg" alt=""/>
          </div>
          <div className="oneArticle__container">
            ...
            <div className="oneArticle__text-container">
              <h1 className="oneArticle__title">
                Ошибка миллионов: никогда не делайте так с зарядным устройством
              </h1>
              <p className="oneArticle__text">
                Когда люди заряжают свои смартфоны, часто забывают извлечь зарядное устройство из розетки, а некоторые даже специально оставляют его подключенным для удобства, чтобы не искать его снова, когда понадобится подзарядить телефон.<br/>
                <br/>
                Однако такая привычка является опасной и должна быть исключена. Важно своевременно извлекать зарядное устройство из розетки, так как его оставление включенным может привести к серьезным последствиям.<br/>
                <br/>
                Каждый владелец устройства должен понимать, что даже если гаджет не подключен, зарядное устройство продолжает потреблять энергию. Поэтому извлечение его из розетки имеет смысл, по крайней мере, с точки зрения экономии энергии.<br/>
                <br/>
                Хотя на первый взгляд может показаться, что затраты электроэнергии при оставленном зарядном устройстве невелики, на практике, если оно будет оставаться подключенным на длительное время, затраты могут оказаться значительными.<br/>
                <br/>
                Также стоит учесть возможное последствие этой привычки - риск короткого замыкания. Вероятность такого случая невысока, но даже маленький риск может оказаться недопустимым, поэтому лучше не подвергать себя лишним рискам<br/>
                <br/>
              </p>
            </div>
          </div>
